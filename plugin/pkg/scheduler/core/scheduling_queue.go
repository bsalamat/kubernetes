...
type SchedulingQueue interface {
	...
	AddUnschedulable(pod *v1.Pod) error
	Pop() (*v1.Pod, error)
	...
	WaitingPodsOnNode(nodeName string) []*v1.Pod
}
...
// WaitingPodsOnNode returns pods that are nominated to run on the given node,
// but FIFO does not support it.
func (f *FIFO) WaitingPodsOnNode(nodeName string) []*v1.Pod {
	return nil
}
...
type UnschedulablePods interface {
	...
	Delete(pod *v1.Pod)
	Update(pod *v1.Pod)
	GetPodsForNode(nodeName string) []*v1.Pod
...
type PriorityQueue struct {
	...
	// receivedMoveRequest is set to true whenever we receive a move request, and
	// is set to false, when we pop a pod. It indicates if we received a move
	// request when a pod was in flight. In such case, we put the pod back into the
	// activeQ if it is determined unschedulable.
...
func (p *PriorityQueue) Add(pod *v1.Pod) error {
	...
	if err != nil {
		...
		// We don't expect the pod to be in the unschedulableQ, but delete it just in case.
...
// Delete deletes the item from either of the two queues.
...
// movePodsWithMatchingAffinityToActiveQueue moves unschedulable pods which have
// any affinity term that matches "pod" to the scheduling queue.
func (p *PriorityQueue) getPodsWithMatchingAffinityTerm(pod *v1.Pod) []*v1.Pod {
	p.lock.RLock()
	defer p.lock.RUnlock()
	podsToMove := []*v1.Pod{}
	for _, p := range p.unschedulableQ.pods {
		affinity := p.Spec.Affinity
		if affinity != nil && affinity.PodAffinity != nil {
			terms := predicates.GetPodAffinityTerms(affinity.PodAffinity)
			for _, term := range terms {
				namespaces := priorityutil.GetNamespacesFromPodAffinityTerm(p, &term)
				selector, err := metav1.LabelSelectorAsSelector(term.LabelSelector)
				if err != nil {
					glog.Errorf("Error getting label selectors for pod: %v.", p.Name)
				}
				if priorityutil.PodMatchesTermsNamespaceAndSelector(pod, namespaces, selector) {
					podsToMove = append(podsToMove, p)
				}
			}
		}
	}
...
// Exists returns true if the given pod is found in the map.
...
// GetPodsForNode returns a list of unschedulable pods whose NominatedNodeNames
// are equal to the given nodeName.
func (u *UnschedulablePodsMap) GetPodsForNode(nodeName string) []*v1.Pod {
	var pods []*v1.Pod
