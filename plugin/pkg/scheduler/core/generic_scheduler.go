...
func (g *genericScheduler) getLowerPriorityNominatedPods(pod *v1.Pod, nodeName string) []*v1.Pod {
	pods := g.schedulingQueue.WaitingPodsOnNode(nodeName)
	if len(pods) == 0 {
		glog.Infof("BOBBY: ----- no pods to clear")
		return nil
	}
...
func addNominatedPods(podPriority int32, meta algorithm.PredicateMetadata,
	...
	*schedulercache.NodeInfo) {
	if queue == nil {
		// This may happen only in unit-tests.
		return false, meta, nodeInfo
	}
	nominatedPods := queue.WaitingPodsOnNode(nodeInfo.Node().Name)
...
func podFitsOnNode(
	...
	// the node added to meta and nodeInfo. If there are such pods and if all
	// predicates succeed in this pass, we run them again when these nominated pods
	// are not added. This second pass is necessary because some predicates such
	// as inter-pod affinity may not pass without the nominated pods.
	for i := 0; i < 2; i++ {
		metaToUse := meta
		nodeInfoToUse := info
		if i == 0 {
